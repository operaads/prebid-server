...
func (cfg *Configuration) setDerivedDefaults() {
	...
	setDefaultUsersync(cfg.Adapters, openrtb_ext.BidderOperaads, "https://t-odx.op-mobile.opera.com/pbs/sync?gdpr={{.GDPR}}&us_privacy={{.USPrivacy}}&gdpr_consent={{.GDPRConsent}}&r="+url.QueryEscape(externalURL)+"%2Fsetuid%3Fbidder%3Doperaads%26gdpr%3D{{.GDPR}}%26gdpr_consent%3D{{.GDPRConsent}}%26uid%3D%24%7BUID%7D")
...
func SetupViper(v *viper.Viper, filename string) {
	...
	v.SetDefault("adapters.operaads.endpoint", "https://s-adx.op-mobile.opera.com/ortb/v2/{{.PublisherID}}?ep={{.AccountID}}")
